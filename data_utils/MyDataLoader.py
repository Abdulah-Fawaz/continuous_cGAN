...
minima = torch.Tensor([0.0,-0.4, 0.1, -10.0] )
maxima = torch.Tensor([2.2, 0.5,2.0, 10.0 ] )
# minima = torch.Tensor([0.0,-0.5, 0.5, -10.0] )
# maxima = torch.Tensor([2.2, 0.5,1.5, 10.0 ] )
minima = torch.Tensor([0.0,-0.5, 0.5, -10.0] )
maxima = torch.Tensor([2.2, 0.5,1.5, 10.0 ] )
...
dmri_means = torch.Tensor([0.10292342, 0.0010362, 0.08419238, 0.15764983])
dmri_stds = torch.Tensor([0.05, 0.0003837, 0.0377524, 0.09069089])
...
class My_dmri_Data(torch.utils.data.Dataset):
...
#                right = True
    ...
    def __getitem__(self, idx):
        ...
        image_gifti = [nb.load(self.directory + '/'+individual_filename+'.func.gii').darrays for individual_filename in filename]
        ...
        if self.normalisation != None:
            if self.normalisation == 'std':
                for i in range(len(image)):
                    ...
                    image[i] = ( image[i] - dmri_means[i%len(dmri_means)].item( )) / dmri_stds[i%len(dmri_stds)].item()
...
class My_dmri_Data_Graph(torch.utils.data.Dataset):
...
class My_dHCP_Data_Graph_parity(torch.utils.data.Dataset):
...
class dHCP_GAN_Age_Graph(torch.utils.data.Dataset):
    ...
    def __init__(self, input_arr, warped_files_directory, unwarped_files_directory,  edges, rotations = False,
        ...
        self.input_arr = input_arr
#        prem_arr_1 = input_arr[input_arr[:,-1]<=37]
        self.prem_arr = input_arr[input_arr[:,-1]<=36]
        
        self.term_arr = input_arr[input_arr[:,-1]>=41]
...
#           
    ...
    def __getitem__(self, idx):
        ...
        image_L_filename = self.__genfilename__(idx,False, False) #false means not prem

        image_R_filename = self.__genfilename__(random.randint(0,L_prem-1), True, False) #true means prem
...
#        label = self.label[idx]
        ...
        sample = Data(x_A = image_L.permute(1,0), x_B = image_R.permute(1,0), edge_index = self.edges)

        return sample
    
    
class dHCP_GAN_Age_Graph_conditional(torch.utils.data.Dataset):
    ...
    def __init__(self, input_arr, warped_files_directory, unwarped_files_directory,  edges, rotations = False,
        ...
        self.prem_arr = input_arr[input_arr[:,2]<=35]
        
        self.term_arr =  input_arr[input_arr[:,2]>=41]
...
#           
    ...
    def __getitem__(self, idx):
        ...
        prem_index = random.randint(0,L_prem-1)
        image_R_filename = self.__genfilename__(prem_index, True, False)
            
        term_scan_age = torch.Tensor([self.term_arr[idx,1 ]]).repeat(162,1)
        prem_scan_age = torch.Tensor([self.prem_arr[prem_index,1]]).repeat(162,1)
...
#        image_L = torch.cat([image_L, term_scan_age], dim=0)
#        image_R = torch.cat([image_R, prem_scan_age], dim=0)

        sample = Data(x_A = image_L.permute(1,0), x_B = image_R.permute(1,0), edge_index = self.edges, meta_A = term_scan_age, meta_B = prem_scan_age)

        return sample
    
    
  
class dHCP_GAN_Age_Graph_long(torch.utils.data.Dataset):

    def __init__(self, input_arr, warped_files_directory, unwarped_files_directory,  edges, rotations = False,
                 number_of_warps = 0,  smoothing = False, normalisation = None, projected =False, sample_all = True, output_as_torch = True, *args):
        ...
        self.term_arr =  input_arr[input_arr[:,-1]>=40]
    ...
    def __genfilename__(self,idx, warp_choice,prem, right):
...
#           
    ...
    def __getitem__(self, idx):
        ...
        L_term = len(self.term_arr)
        L_prem = len(self.prem_arr)
        full_L_term = self.__len__()
        
        if self.number_of_warps!=0:
            full_L_prem = L_prem *self.number_of_warps
        else:
            full_L_prem = L_prem
        
        L_term_idx, L_term_warp_choice = idx % L_term, str(idx//L_term)
        
        prem_idx = random.randint(0,full_L_prem-1)
        L_prem_idx, L_prem_warp_choice = prem_idx % L_prem, str(prem_idx//L_prem)

        
        image_L_filename = self.__genfilename__(L_term_idx,L_term_warp_choice,False, False) #false means not prem

        image_R_filename = self.__genfilename__(L_prem_idx, L_prem_warp_choice,True, False) #true means prem
