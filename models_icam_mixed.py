...
class GraphNet_Generator(nn.Module):
    
    def __init__(self, in_channels, c_dim, age_dim,conv_style = graphconv, device='cuda'):
        super(GraphNet_Generator, self).__init__()   
        self.conv_style = conv_style
        self.device = device
        ...
        self.fc1 = nn.Linear(128*642, c_dim)
    ...
    def encode(self, x , bs):
        ...
        x = x.reshape(bs,-1)
        encoding = self.fc1(x)
        noisy_encoding += torch.randn_like(encoding).to(encoding.device)
        
        
        return noisy_encoding
    
    def decode(self, encoding, age,bs):
        age = self.fc_age(age)
        y = torch.cat([encoding, age],dim=1)
        x = self.relu(self.fc2(y))
    ...
    def forward(self,x, age):
         ...
         encoding  = self.encode(x,bs)

         output = self.decode(encoding,bs,age)   
