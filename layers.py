...
class pool_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pooling_type == "max":
            x = torch.max(x, 2)
            print(x[0].size())
            assert(x[0].size()[:-1] == torch.Size([num_nodes, feat_num]))
            return x[0], x[1]
...
class upconv_layer(nn.Module):
    ...
    def __init__(self, in_feats, out_feats, upconv_top_index, upconv_down_index):
        ...
        self.upconv_top_index = upconv_top_index
        self.upconv_down_index = upconv_down_index
        self.weight = nn.Linear(in_feats, 19 * out_feats)
...
#        x = self.weight(x)
        x = x.view(len(x) * 19, self.out_feats,-1)
...
class upsample_interpolation(nn.Module):
    ...
    def forward(self, x):
        ...
        num_nodes = x.size()[0] * 4 - 6
        feat_num = x.size()[1]
        x1 = x[self.parent_nodes].view(num_nodes - x.size()[0], feat_num, 2)
        x1 = torch.mean(x1, 2)
        x = torch.cat((x,x1),0)
