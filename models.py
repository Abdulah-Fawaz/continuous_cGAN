...
class Graph_GAN(nn.Module):
    def __init__(self, num_features, conv_style=chebconv,activation_function=nn.LeakyReLU(), in_channels = 4, latent_dim = 100, device='cuda'):
        ...
        self.fc_decode = nn.Linear(latent_dim, 162)
...
#        self.conv6 = conv_style(num_features[3], num_features[2])
    ...
    def forward(self, encoding ,batch_size):
        ...
        y = y.reshape(batch_size*162,-1)
        # y = self.fc_decode(encoding)
        y = self.conv_decode(y,expand_edge_index(edge_index_2, batch_size-1).to(self.device), expand_pseudo_index(pseudo_2, batch_size-1).to(self.device))
#        print(y.shape)
...
class ResBlock_gmmconv(nn.Module):
    ...
    def __init__(self, channels, conv_style=gmmconv,norm=True,activation_function=nn.GELU(), device='cuda'):
...
class MoNetEncoder(nn.Module):
    ...
    def __init__(self, num_features, conv_style=gmmconv,activation_function=nn.LeakyReLU(), in_channels = 4, latent_dim = 100, device='cuda'):
        ...
        self.conv3 = conv_style(num_features[1], num_features[2])
        self.conv4 = conv_style(num_features[2], num_features[3])
        self.res1 = ResBlock_gmmconv(num_features[3],norm=False)
...
class Monet_Encoder(nn.Module):
    def __init__(self, num_features, conv_style=gmmconv,activation_function=nn.LeakyReLU(), in_channels = 4, latent_dim = 100, device='cuda'):
        super(Monet_Encoder, self).__init__()
        self.conv_style = conv_style
        self.device = device
        self.in_channels = in_channels
        ...
        self.conv_mu = conv_style(num_features[3], 1)
        self.conv_logvar = conv_style(num_features[3], 1)
...
#        print('Decoded shape: ', print(x_f.shape))
        ...
        encoding_mu = self.conv_mu(x,expand_edge_index(edge_index_2, batch_size-1).to(self.device), expand_pseudo_index(pseudo_2, batch_size-1).to(self.device)).reshape(batch_size, -1)
        encoding_var = self.conv_logvar(x,expand_edge_index(edge_index_2, batch_size-1).to(self.device), expand_pseudo_index(pseudo_2, batch_size-1).to(self.device)).reshape(batch_size, -1)
        
        
        z = self.reparametrize(encoding_mu, encoding_var)
        
        return z, encoding_mu, encoding_var, batch_size
    
class Monet_Decoder(nn.Module):
    def __init__(self, num_features, conv_style=gmmconv,activation_function=nn.LeakyReLU(), in_channels = 4, latent_dim = 100, device='cuda'):
        super(Monet_Decoder, self).__init__()   
        self.conv_style = conv_style
        self.device = device
        self.in_channels = in_channels        
        self.conv_decode = conv_style(1,num_features[3])
...
class Monet_GAN(nn.Module):
    def __init__(self, num_features, conv_style=gmmconv,activation_function=nn.GELU(), in_channels = 4, latent_dim = 100, device='cuda'):
        super(Monet_GAN, self).__init__()   
        self.conv_style = conv_style
        self.device = device
        self.in_channels = in_channels        
        self.conv_decode = conv_style(1,num_features[3])
        ...
        self.res5 = ResBlock_gmmconv(num_features[3],norm=False)
        self.res6 = ResBlock_gmmconv(num_features[3], norm=False)
        self.res7 = ResBlock_gmmconv(num_features[3], norm=False)
        self.res8 = ResBlock_gmmconv(num_features[3], norm=False)
        self.res9 = ResBlock_gmmconv(num_features[0],norm=False)
