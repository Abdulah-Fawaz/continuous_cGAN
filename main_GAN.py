...
from models import Monet_GAN, Discriminator,Discriminator2, monet_variational_upsample_batched, Monet_Decoder, Monet_Encoder
...
generator = Monet_GAN(num_features = features, in_channels= in_channels, latent_dim = latent_dim).to(device)

discriminator = Discriminator2(num_features = [32,32,64,128], in_channels= in_channels).to(device)
...
optimizer_G = torch.optim.Adam(generator.parameters(), lr = learning_rate, weight_decay= 1e-5)
...
print('Starting...')
divergence = 'GAN'
gen_loss =QLOSS(divergence)
dis_loss =VLOSS(divergence)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(train_loader):
        ...
        g_loss = -1* dis_loss(discriminator(gen_data))
        ...
        if counter % update_D_freq ==0:
            ...
            real_loss = -1 * dis_loss(discriminator(data))
            real_loss.backward(retain_graph=True)
            
            fake_loss = -1 * gen_loss(discriminator(gen_data))
            fake_loss.backward()
