...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug  7 22:26:54 2022
...
# full_dataset_arr = np.load('data/' + 'scan_age' + '/full.npy', allow_pickle = True)
...
from markovian_models import GraphNet_Markovian_Generator, GraphNet_Markovian_Discriminator,MoNet_Markovian_Discriminator_Simple, GraphNet_Markovian_Discriminator_Simple, MoNet_Markovian_Generator, MoNet_Markovian_Discriminator
...
discriminator = MoNet_Markovian_Discriminator_Simple(len(mode),age_dim = 1,device=device).to(device)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(train_loader):
        ...
        im1_recon_loss = recon_loss(im3, im1)  * 5
        
        im2_real_loss = adversarial_loss(discriminator(im2, A2_v), valid)           
        im3_real_loss = adversarial_loss(discriminator(im3, A1_v), valid)           
        

        
        gloss = im1_recon_loss + im2_real_loss + im3_real_loss
        ...
        if i % d_update_freq==0 :
           ...
           # fake_loss4 = adversarial_loss(discriminator(im3,A3_v), fake)    
            ...
            d_loss = real_image_true  + fake_loss3  + fake_loss5
    ...
    total_gen_losses.append(np.mean(g_losses))
    total_disc_losses.append(np.mean(d_losses))
    ...
    if epoch % 50 == 0:
        ...
        torch.save(model, '/home/fa19/Documents/neurips/MoNet_2cycle_both_generator_model_final')
                
        torch.save(discriminator, '/home/fa19/Documents/neurips/MoNet_2cycle_both_discriminator_model_final')
                
        torch.save(model.state_dict(), '/home/fa19/Documents/neurips/MoNet_2cycle_both_generator_model_final_sd')
                
        torch.save(discriminator.state_dict(), '/home/fa19/Documents/neurips/MoNet_2cycle_both_discriminator_model_final_sd')
                



# np.save('total_disc_losses_monet', total_disc_losses)
# np.save('total_gen_losses_monet', total_gen_losses)



import matplotlib.pyplot as plt
fig = plt.figure()
LL = len(total_gen_losses)
plt.plot(np.arange(LL), total_disc_losses, label = 'discriminator loss')
plt.plot(np.arange(LL), total_gen_losses, label = 'generator loss')
plt.ylabel('Loss')
plt.xlabel('Epoch')
plt.legend()
plt.show()
...
#### NB CHANGE THE ONE YOURE LOADING XXXXXX ##############
# model = torch.load('/home/fa19/Documents/neurips/MoNet_3cycle_generator_model_final')

##### NB CHANGE HTE ABOVE CHANGE THE ABOVE####
...
for idxs in all_shared:
    ...
    for num in np.arange(32,45):
        ...
        save_as_metric(im1_np[0], '2cycle_MoNet/test/val_original'+'_'+str(true_age_1[0].item())+'_'+str(name))
        
        save_as_metric(im2[0], '2cycle_MoNet/test/val_fake'+'_'+str(new_age[0].item())+'_'+str(name))
    ...
    save_as_metric(im2_np[0], '2cycle_MoNet/test/val_original_secondscan'+'_'+str(true_age_2[0].item())+'_'+str(name))
