...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  4 12:29:40 2022
    ...
    for i, data in enumerate(train_loader):
        ...
        if i % 10 == 0:
            ...
            for b in range(min(3,bs)):
                save_as_metric(im1[b], '2cycle/im1'+'_'+str(i)+'_'+str(b))
                
                save_as_metric(im2[b], '2cycle/im2'+'_'+str(i)+'_'+str(b))
                save_as_metric(im3[b], '2cycle/im3'+'_'+str(i)+'_'+str(b))
...
# im1 =  data['x'][:,mode].to(device) 
    ...
    for num in [32,35,38,40,41,43,45]:
        ...
        save_as_metric(im1_np[0], '2cycle/val_original'+'_'+str(A1[0].item())+'_'+str(name))
        
        save_as_metric(im2[0], '2cycle/val_fake'+'_'+str(A2[0].item())+'_'+str(name))
    print(i, ' Complete')
    time.sleep(2)
    if i == 20:
        break
...
# torch.save(model, '/home/fa19/Documents/neurips/2cycle_generator_model_final')
        
# torch.save(discriminator, '/home/fa19/Documents/neurips/2cycle_discriminator_model_final')
        
# torch.save(model.state_dict(), '/home/fa19/Documents/neurips/2cycle_generator_model_final_sd')
        
# torch.save(discriminator.state_dict(), '/home/fa19/Documents/neurips/2cycle_discriminator_model_final_sd')
...
# D = everything.__getitem__(idx1)


# change_map = netG_B2A(D.x[:,[0,1]].to(device), D.edge_index.to(device), data.batch.to(device)) 

# save_as_metric(change_map.detach().cpu().numpy(), str(subject) + '_2low_to_high_map')

# #new_image = D.x[:,[0,1]]+change_map.detach().cpu()

# #save_as_metric(new_image.numpy(), str(subject) + '_low_to_high')



# idx2 = idxs[1]

# D = everything.__getitem__(idx2)


# change_map = netG_A2B(D.x[:,[0,1]].to(device), D.edge_index.to(device), data.batch.to(device)) 

# #    save_as_metric(change_map.detach().cpu().numpy(), str(subject) + '_2high_to_low_map')

# #new_image = D.x[:,[0,1]]+change_map.detach().cpu()

# #save_as_metric(new_image.numpy(), str(subject) + '_high_to_low')


# print(everything_arr[idxs[:2]])
...
for num in np.arange(32,45):
    ...
    save_as_metric(im1_np[0], '2cycle/test/val_original'+'_'+str(true_age_1[0].item())+'_'+str(name))
    
    save_as_metric(im2[0], '2cycle/test/val_fake'+'_'+str(new_age[0].item())+'_'+str(name))
...
save_as_metric(im2_np[0], '2cycle/test/val_original_secondscan'+'_'+str(true_age_2[0].item())+'_'+str(name))
