...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 23 22:49:35 2022
...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 18 13:38:53 2022
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(train_loader):
        ...
        A4_v = torch.randint(20,50, (bs,1)).to(device).float()
        A5_v = torch.randint(20,50, (bs,1)).to(device).float()
        ...
        im1_recon_loss = recon_loss(im6, im1)  * 5
        
        im2_real_loss = adversarial_loss(discriminator(im2, A2_v), valid)           
        im3_real_loss = adversarial_loss(discriminator(im3, A3_v), valid)           
        im4_real_loss = adversarial_loss(discriminator(im4, A4_v), valid)           
        im5_real_loss = adversarial_loss(discriminator(im5, A5_v), valid)           
        

        
        gloss = im1_recon_loss + im2_real_loss + im3_real_loss + im4_real_loss + im5_real_loss
        ...
        if i % d_update_freq==0 :
            ...
            # these losses are to discriminate the generated images
            
            fake_gloss1 = adversarial_loss(discriminator(im2,A2_v), fake)    
            fake_gloss2 = adversarial_loss(discriminator(im3,A3_v), fake)    
            fake_gloss3 = adversarial_loss(discriminator(im4,A4_v), fake)    
            fake_gloss4 = adversarial_loss(discriminator(im5,A5_v), fake)    

            # these losses are to discriminate the real image frmo the fake ages

            fake_rloss1 = adversarial_loss(discriminator(im1,A2_v), fake)    
            fake_rloss2 = adversarial_loss(discriminator(im1,A3_v), fake)     
            fake_rloss3 = adversarial_loss(discriminator(im1,A4_v), fake)     
            fake_rloss4 = adversarial_loss(discriminator(im1,A5_v), fake)     
            
            # d_loss = real_image_true + fake_loss1 + fake_loss2 + fake_loss3 + fake_loss4 + fake_loss5 + fake_loss6

            d_loss = real_image_true  + fake_gloss1 + fake_gloss2 + fake_gloss3 + fake_gloss4 + fake_rloss1 + fake_rloss2 + fake_rloss3 + fake_rloss4
        ...
        if i % 10 == 0:
            ...
            im4 = im4.reshape(bs,-1,len(mode)).detach().cpu().numpy()
            im5 = im5.reshape(bs,-1,len(mode)).detach().cpu().numpy()
            ...
            for b in range(min(3,bs)):
                ...
                save_as_metric(im4[b], 'im4'+'_'+str(i)+'_'+str(b))
                save_as_metric(im5[b], 'im5'+'_'+str(i)+'_'+str(b))
...
torch.save(model, '/home/fa19/Documents/neurips/5cycle_double_generator_model_final')
        
torch.save(discriminator, '/home/fa19/Documents/neurips/5cycle_double_discriminator_model_final')
        
torch.save(model.state_dict(), '/home/fa19/Documents/neurips/5cycle_double_generator_model_final_sd')
        
torch.save(discriminator.state_dict(), '/home/fa19/Documents/neurips/5cycle_double_discriminator_model_final_sd')
        
# model = torch.load('/home/fa19/Documents/neurips/5cycle_double_generator_model_final')
...
for idxs in all_shared:
    ...
    for num in np.arange(32,45):
        ...
        save_as_metric(im1_np[0], '5cycle/test/val_original'+'_'+str(true_age_1[0].item())+'_'+str(name))
        
        save_as_metric(im2[0], '5cycle/test/val_fake'+'_'+str(new_age[0].item())+'_'+str(name))
