...
from models import monet_variational_upconv_batched, Discriminator,Discriminator2, monet_variational_upsample_batched, Monet_Decoder, Monet_Encoder
...
discriminator_model = Discriminator2(num_features = [16,32,64,128], in_channels= in_channels).to(device)
...
for layer in discriminator_model.children():
       if hasattr(layer, 'reset_parameters'):
           layer.reset_parameters() 
...
optimizer_encoder = torch.optim.Adam(encoder.parameters(), lr = learning_rate, weight_decay= 1e-5)
optimizer_decoder = torch.optim.Adam(decoder.parameters(), lr = learning_rate, weight_decay= 1e-5)

optimizer_D = torch.optim.Adam(discriminator_model.parameters(), lr = learning_rate, weight_decay= 1e-5)

def criterion(x, y, mean, log_var):

    bceloss = nn.BCELoss(reduction='sum')(x,y)
    # l1loss = nn.L1Loss(reduction = 'mean')(x,y)
...
for epoch in range(num_epochs):
    ...
    print('Epoch ' + str(epoch) + ' ', np.mean(train_loss),np.mean(discriminator_losses) )
    ...
    for i,data in enumerate(val_loader):
        ...
        loss = criterion(reconstruction, data.x, mean, var)#        if i ==0:
...
torch.save(encoder.state_dict(), 'vae-gan-results/saved_models/encoder_vae_gan_statedict')
torch.save(decoder.state_dict(), 'vae-gan-results/saved_models/decoder_vae_gan_statedict')
torch.save(discriminator_model.state_dict(), 'vae-gan-results/saved_models/discriminator_vae_gan_statedict')
